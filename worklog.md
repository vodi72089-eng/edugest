...
Task: Cloner https://github.com/vodi72089-eng/edugest, identifier les erreurs contraires aux promesses de l'app, les corriger, auditer la sécurité avec Semgrep et installer les MCP (playwright, memory-bank, semgrep, codev)

Work Log:
- Cloné le dépôt edugest dans /home/z/edugest-clone (47 MB, 5 commits récents dont audits précédents)
- Copié l'intégralité (src, prisma, mini-services, public, docs…) dans /home/z/my-project via rsync (hors .git/node_modules), puis assets PNG publics
- Créé .env (DATABASE_URL SQLite, WHATSAPP_API_KEY, PAYMENT_KEYS_SECRET…), bun install (19 deps ajoutées dont bcryptjs, qrcode, pdfkit, web-push…), prisma generate + db push (base sync)
- Démarré le dev server Next.js 16.1.3 (port 3000) — HTTP 200
- Analyse approfondie via agent Explore : 18 écarts promesses vs code identifiés (chaîne WhatsApp, gating forfaits, IDOR, pricing…)
- Corrections WhatsApp : quota basé sur WhatsappMessageLog (reset mensuel réel), recordWhatsappMessage appelé sur chaque envoi, routage automatique vers l'API Meta Cloud perso (sendViaWhatsappApi/sendDocumentViaWhatsappApi) quand configurée, schoolId propagé aux 12 points d'appel
- Sécurité : IDOR /api/whatsapp-config (rôles + scellage école), GET settings-approval restreint, SVG servi attachment+CSP sandbox (anti-XSS), token Meta chiffré AES-256-GCM, subscription/request réservé admins, SECRETARY retiré de l'import DB, tag GCM 16 octets vérifié
- Gating forfaits : quota élèves appliqué à l'import-db (studentSlots), comptes parents gated canManageParentAccounts, bulletins gated report_cards pour le staff, ADMIN_FREEMIUM ajouté à API_ROLE_MAP
- Promesses marketing alignées sur le backend : Pricing.tsx + DEFAULT_TIERS api/pricing + ligne DB PricingPlan (Professionnel : « App mobile dédiée » → « Analytique avancée ») ; FreemiumAdminDashboard répare son fetch 404 (→ GET /api/schools/[id]) et le lien /#pricing
- Seed rendu idempotent (safeCreateUser, P2002 → réutilise l'existant)
- MCP installés dans .mcp.json : playwright (@playwright/mcp), memory-bank (memory-bank-mcp), semgrep (~/.semgrep-mcp-venv/bin/semgrep-mcp v0.9.0 + CLI 1.177.0), codev (codev-ai/codevhub 0.6.4 global)
- Scan Semgrep (6 suites de règles) : 16 findings, 1 réel corrigé (GCM), 2 faux positifs documentés, reste = artefacts tooling ; scan manuel eval/child_process/dangerouslySetInnerHTML clean
- Vérification navigateur (agent-browser) : landing OK, login Super Admin → dashboard 23 vues, login SCHOOL_ADMIN OK, /api/whatsapp/usage renvoie les quotas réels, IDOR re-testé (config d'autrui inaccessible, POST scellé), settings-approval TEACHER → 403, section tarifs rend les valeurs corrigées
- tsc --noEmit : 0 erreur ; ESLint fichiers modifiés : propre

Stage Summary:
- 18+2 écarts promesses/failles corrigés et documentés dans RAPPORT-SECURITE-CORRECTIONS.md
- Chaîne WhatsApp entièrement fonctionnelle (quota mensuel réel, usage suivi, API Meta perso routée, token chiffré)
...
Task: 4 vues (Documentation, CorporatesAdmin, PlatformEmails, Logs)
Work Log:
- Lecture du worklog (contexte v0.2.3/v0.2.4, patterns Discipline/Support) + étude des références de style : CorporateSpaceView.tsx (cartes, en-têtes, StatMini), SupportView.tsx (AppSelect, pills, boutons or), AppSelect.tsx (props value/onChange/options/className/triggerClassName), constants (GOLD/TEXT_PRIMARY/TEXT_MUTED_LUXE/SUCCESS/DANGER/WARNING) et helpers (formatDate).
- src/components/views/DocumentationView.tsx : NOUVEAU — documentation in-app 100 % statique (0 appel API). 8 sections riches en données typées (SECTIONS : id/title/icon/summary/keywords/blocks p|list|note). Recherche locale (filtre titre+résumé+keywords+texte des blocs), sommaire cliquable (scrollIntoView smooth + scroll-mt-20, sticky en lg), cartes rounded-2xl border, pastilles spécifiques Liste Blanche/Grise/Noire (SUCCESS/WARNING/DANGER) et chips des 6 forfaits (FREEMIUM→CORPORATE, PREMIUM/CORPORATE en or). Notes tone gold/success/warning/danger. Vide de recherche → « Aucun résultat ».
- src/components/views/CorporatesAdminView.tsx : NOUVEAU — admin plateforme « Entreprises ». GET /api/corporates + GET /api/schools?limit=200. Formulaire création (Nom*/Ville/contact nom+email+tél/Notes) + multi-select d'écoles en checkboxes stylées avec recherche locale (selectedSchools: string[]) + section compte (Nom*/Email*/Téléphone*/Mot de passe* avec bouton « Générer » : 12 car. alphanum via crypto.getRandomValues, champ visible mono). POST /api/corporates { name, contactName, contactEmail, contactPhone, city, notes, schools[], user{name,email,phone,password} }. Liste en cartes expand inline : écoles rattachées avec « Détacher » (DELETE /api/corporates/{id}/schools?schoolId=…), AppSelect « Rattacher une école » (écoles non rattachées → POST /api/corporates/{id}/schools {schoolId}), comptes utilisateurs (nom/email/rôle), Suspendre/Réactiver (PUT /api/corporates/{id} {status}), Supprimer en 2 clics (« Confirmer ? » auto-reset 3 s via useEffect timeout). Pills ACTIVE/SUSPENDED, nb écoles, nb comptes, badge openTickets (or si > 0). Vide : « Aucun corporate — créez votre premier client multi-écoles ».
- src/components/views/PlatformEmailsView.tsx : NOUVEAU — 2 onglets (boutons comme SupportView). Onglet Adresses : GET /api/platform-emails?tab=addresses → cartes éditables (label, description, input adresse locale préservé au reload via setEdited fonctionnel), « Tester » (input destinataire → POST /api/platform-emails {fromKey,to} → toast j.data.status==='SENT' ? 'Envoyé' : 'SIMULÉ (Resend non configuré)'), bouton « Enregistrer » global (PUT {addresses:[{key,address}]}), bandeau Resend configuré/activé vs « les envois sont SIMULÉS (journalisés) ». Onglet Boîte d'envoi : GET /api/platform-emails → compteurs en chips colorés (SENT vert / SIMULATED ambre / FAILED rouge) + tableau max-h-[60vh] overflow-y-auto (date+heure fr-FR, à, sujet, template, fromKey, pill statut avec title=error sur FAILED), responsive grid → stack mobile, refresh.
- src/components/views/LogsView.tsx : NOUVEAU — GET /api/logs → { data, hermes, viewer }. Carte Agent Hermes : si hermes null → texte « Non configuré — en développement, les logs restent en local… signature HMAC-SHA256 » + bouton « Configurer » ; si présent → pill ACTIF/INACTIF + URL tronquée (truncateUrl 52 car.) + « Modifier » ; formulaire (webhookUrl, secret placeholder « laissez vide pour conserver », checkbox stylée activée) → PUT /api/logs {webhookUrl, secret, enabled}. Filtres : AppSelect des types d'action déduits dynamiquement (valeurs uniques de action, triées) + recherche texte userName/details. Liste max-h-[60vh] : icône par préfixe d'action (AUTH→Lock, CORP→Building2, GRANT→ListChecks, EMAIL→Mail, TICKET→Ticket, AGENT→Bot, sinon ScrollText), pill action mono, userName+userRole, details, meta en <details> « Détails techniques » (pre JSON.stringify(meta,null,2)), formatDate + toLocaleTimeString('fr-FR').
- Conventions respectées : 'use client', export default sans props, authFetch, constantes couleur inline, cartes rounded-2xl border oklch(92% 0.01 175), boutons primaires fond GOLD + #0a0f0d, h1 text-2xl sm:text-3xl font-extrabold tracking-tighter edu-heading-display, skeletons animate-pulse oklch(95% 0.01 175), grid responsive grid-cols-1 sm/md/lg, inputs focus:border-[oklch(72%_0.15_65)], apostrophes typographiques ’ écrites directement dans le JSX, aucun composant shadcn Card/Button.
- Vérification : bunx tsc --noEmit → 0 erreur sur les 4 fichiers (grep filtré vide) ; bunx eslint ciblé sur les 4 fichiers → 0 problème. Aucun autre fichier touché.
Stage Summary:
- 4 vues client prêtes à brancher dans le routeur de vues de page.tsx : 'documentation' (DocumentationView, statique), 'corporates' (CorporatesAdminView), 'platform-emails' (PlatformEmailsView), 'activity-logs' (LogsView) — attendues par le menu admin plateforme.
- Contrats API consommés : GET/POST /api/corporates, PUT/DELETE /api/corporates/{id}, POST/DELETE /api/corporates/{id}/schools, GET /api/schools?limit=200 ({data:[{id,name,shortName,city,…}]}), GET (?tab=addresses)/POST/PUT /api/platform-emails (addresses+resend / outbox+counts), GET/PUT /api/logs ({data, hermes{enabled,webhookUrl}|null}).
- Tolérance de forme sur les corporates : nb écoles/comptes acceptés soit en tableaux (schools/users), soit en _count — helpers schoolCount/userCount.
- Simplifications assumées : DocumentationView sans authFetch/formatDate/AppSelect/toast (aucun appel API, imports inutilisés évités) ; confirmation de suppression en 2 clics inline (pas de modale) ; mot de passe corporate en champ texte visible avec générateur.
- tsc 0 erreur et eslint 0 problème sur les 4 fichiers ; lint baseline globale intacte (aucun autre fichier modifié).

---
Task ID: CORPORATE-1
Agent: Main Agent (Z.ai Code) + full-stack-developer (Task 8-b : 4 vues)
Task: Compte corporate distinct multi-écoles + emails plateforme Resend + documentation + agent IA support + support client (tickets) + logs prêts pour agent Hermes (prod) + passage d'école envoyé manuellement par l'admin plateforme.

Work Log:
- Prisma : +7 modèles (Corporate, CorporateSchool, CorporateUser, SupportTicket, TicketMessage, EmailMessage, FeatureGrant) ; AuditLog EXISTANT étendu (schoolId, meta, index) — doublon évité ; db push OK ; relations User/School ajoutées
- auth.ts : rôles CORPORATE_ADMIN (niveau 45, schoolId null) + SUPPORT_AGENT (niveau 30) avec permissions dédiées (corporate:*, support:*, logs:read) ; SCHOOL_ADMIN + support:read/create ; ROLE_CREATION_MATRIX couvert par SAG ['*']
- src/lib/audit.ts : logAudit() — écriture AuditLog toujours + relais Hermes fire-and-forget (webhook + HMAC-SHA256 X-EduGest-Signature, config GlobalApiConfig HERMES_AGENT_CONFIG) ; inactif en dev
- src/lib/platform-email.ts : NOS EMAILS — noreply/support/contact@edugest.app éditables (GlobalApiConfig PLATFORM_EMAIL_ADDRESSES) ; sendPlatformEmail() journalise dans EmailMessage (SENT via Resend / SIMULATED en dev / FAILED) ; sendEmailViaResend() étendu avec fromOverride ; hasFeatureGrant()
- API : /api/corporates (GET SAG+support, POST création w/ écoles+compte+email bienvenue) ; /api/corporates/[id] (PUT statut, DELETE protégé) ; /api/corporates/[id]/schools (attach/détach + audit) ; /api/corporate/me (stats agrégées par école : élèves/classes/personnel/encaissements/incidents/tickets) ; /api/support/tickets (GET cloisonné par rôle, POST w/ accusé support@) ; /api/support/tickets/[id] (fil, réponses CLIENT/SUPPORT, réouverture auto, PATCH statut/priorité/assign) ; /api/support/agent (LLM z-ai-web-dev-sdk backend, prompt produit EduGest, 20 msg/5min, audit AGENT_CHAT) ; /api/logs (GET logs:read + hermes masqué, PUT config Hermes SAG) ; /api/platform-emails (GET adresses+outbox, PUT adresses, POST test) ; /api/feature-grants (GET liste/vérif, POST « envoyer » + emails admins école, PATCH révoque)
- GATE PASSAGE : /api/class-passing + repechage (3 gates) → forfait PREMIUM ET FeatureGrant CLASS_PASSING actif (SAG bypass) ; sans grant → 403 grantRequired
- Front : store.ts UserRole +2 rôles, login → vue d'entrée par rôle (corporate→'corporate', support→'support') ; helpers API_ROLE_MAP + getRoleLabel ; page.tsx : VIEWS_BY_ROLE (corporate/support/docs communs via boucle, SAG +corporates/platform-emails/activity-logs), menus sidebar (Entreprises/Emails plateforme/Journal d'activité SAG ; Espace Corporate ; Support client ; Support+Documentation injectés avant « Mon profil » pour TOUS), switch vues x6, ClassPassingView état grantLocked (carte « pas encore activé »), canAccessView early-return comptes hors école
- Vues : CorporateSpaceView (totaux agrégés + cartes écoles w/ stats, statuts pills) + SupportView (tickets cloisonnés, fil CLIENT/SUPPORT/AGENT IA, création, filtres handler, chat Agent IA streaming-like) par agent principal ; DocumentationView (8 sections, recherche, sommaire), CorporatesAdminView (création complète + attach/détach + suspend/delete 2-clics), PlatformEmailsView (adresses éditables + test + outbox pills), LogsView (Hermes config + filtres action + meta JSON) par sous-agent (Task 8-b) — tsc/eslint 0 sur leurs fichiers
- PlatformControlView : section « Passage d'école — activations » (liste écoles, statut ACTIF/RÉVOQUÉ/NON ACTIVÉ, Envoyer/Révoquer, recherche) — le mécanisme demandé « c'est moi qui l'envoie aux utilisateurs »
- Seed : scripts/seed-corporate.ts (idempotent) → support@edugest.app/support123 + corporate@edugest.app/corporate123 (Groupe Scolaire Lumière SARL, 2 écoles)
- Version 0.3.0 ; lint 68 = baseline inchangée ; tsc 0 erreur

Vérification navigateur (agent-browser) :
- SAG : menus nouveaux présents ; Entreprises → corporate « Groupe Scolaire Lumière SARL » (2 écoles, 1 compte, ACTIVE) + expand (Détacher/Rattacher/Suspendre/Supprimer) ; Emails plateforme → 3 adresses officielles, test envoi → toast « SIMULÉ (Resend non configuré) », Boîte d'envoi → chip SIMULATED:1 + ligne TEST/noreply/admin@edugest.app ; Journal d'activité → entrée EMAIL_SENT + carte Hermes « Non configuré » + config PUT
- Agent IA (SAG) : question « passage de classe » → réponse française correcte (forfait PREMIUM + activation admin plateforme) en ~8s
- Corporate (corporate@edugest.app) : atterrissage direct Espace Corporate — totaux 2 écoles/20 élèves/18 classes/13 personnel/4 950 000 FC/0 tickets + cartes écoles (Lumière PREMIUM ACTIVE…) ; création ticket TCK-2026-VC3YA → toast accusé
- Passage : admin@lumiere.cd (PREMIUM sans grant) → carte « Passage de classe pas encore activé » (verrou API+UI) ; SAG Contrôle plateforme → « Envoyer l'activation » → POST 201, ACTIF ; re-login admin école → vue délibération complète déverrouillée (fenêtre plateforme existante toujours appliquée)
- Mobile 375px : pas de débordement horizontal ; console propre après reload (erreurs « Module not found » = artifacts Fast Refresh antérieurs à la création des fichiers, disparus au reload)
