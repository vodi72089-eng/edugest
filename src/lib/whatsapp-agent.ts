...
export async function sendWhatsAppMessage(phone: string, message: string, schoolId?: string | null): Promise<boolean> {
  ...
  if (schoolId) {
    ...
    if (config && config.isActive) {
      const result = await sendViaWhatsappApi(config, phone, message);
      await recordWhatsappMessage({ schoolId, phone, type: 'message', channel: 'custom_api', ok: result.ok });
      if (!result.ok) console.warn('[WhatsApp Agent] Envoi via API perso échoué:', result.error);
      return result.ok;
    }
  }
  ...
  try {
    ...
    await recordWhatsappMessage({ schoolId, phone, type: 'message', channel: 'agent', ok });
    return ok;
  } catch (error) {
    console.warn('[WhatsApp Agent] Send failed:', error);
    await recordWhatsappMessage({ schoolId, phone, type: 'message', channel: 'agent', ok: false });
    return false;
  }
}
...
export async function sendWhatsAppDocument(params: {
  ...
  if (params.schoolId) {
    ...
    if (config && config.isActive) {
      ...
      if (!result.ok) console.warn('[WhatsApp Agent] Document via API perso échoué:', result.error);
      return result.ok;
    }
  }
  ...
  try {
    ...
    const data = await res.json();
    const ok = data.ok === true;
    await recordWhatsappMessage({ schoolId: params.schoolId, phone: params.phone, type: 'document', channel: 'agent', ok });
    return ok;
  } catch (error) {
    console.warn('[WhatsApp Agent] Document send failed:', error);
    await recordWhatsappMessage({ schoolId: params.schoolId, phone: params.phone, type: 'document', channel: 'agent', ok: false });
    return false;
  }
}
...
async function checkSchoolAgentReady(
  ...
  // Vérification de quota mensuel basée sur le journal des envois (reset automatique chaque mois)
