...
export default function FreemiumAdminDashboard() {
  ...
  useEffect(() => {
    if (userData?.schoolId) {
      // Route réelle : GET /api/schools/[id] renvoie l'école avec _count
      fetch(`/api/schools/${userData.schoolId}`)
        .then(r => r.json())
        .then(data => {
          setStats({
            studentCount: data?._count?.students || 0,
            maxStudents: 250,
            classCount: data?._count?.classes || 0,
          });
        })
        .catch(() => {});
    }
