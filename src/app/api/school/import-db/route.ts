...
// (SECRETARY retiré : opération critique réservée à l'administration.)
const IMPORT_ADMIN_ROLES = ['SUPER_ADMIN_GLOBAL', 'SCHOOL_ADMIN', 'DIRECTION_MATERNELLE', 'DIRECTION_PRIMAIRE', 'DIRECTION_SECONDAIRE'];
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════════
    // SÉCURITÉ FORFAIT : l'import respecte le quota d'élèves du forfait
    // (comme POST /api/students). Sinon une école FREEMIUM pourrait
    // importer des milliers d'élèves et contourner la limite payante.
    const schoolForQuota = await db.school.findUnique({ where: { id: schoolId }, select: { subscriptionTier: true } });
    const tierLimits = getTierLimits(schoolForQuota?.subscriptionTier || 'FREEMIUM');
    const currentStudents = await db.student.count({ where: { schoolId, isArchived: false } });
    let studentSlots = Math.max(0, tierLimits.maxStudents - currentStudents);
    if (studentSlots === 0) {
      summary.errors.push(`Quota d'élèves atteint (${currentStudents}/${tierLimits.maxStudents} pour ${schoolForQuota?.subscriptionTier || 'FREEMIUM'}) — import d'élèves bloqué. Passez au forfait supérieur.`);
    }
