...
export async function GET(req: NextRequest) {
  ...
  // SÉCURITÉ : les demandes de paramètres (contenu JSON sensible : classes à
  // créer/supprimer, infos école, QR) ne sont lisibles que par les rôles
  // habilités. Le secrétaire ne voit que les demandes de QR code.
  const isSettingsRole = SETTINGS_ROLES.includes(user.role)
  const isSecretary = user.role === 'SECRETARY'
  if (!isSettingsRole && !isSecretary) {
  ...
  const approvals = await db.settingsApproval.findMany({
    where: {
      ...
      status,
      ...(isSecretary ? { changeType: 'qr_create' } : {}),
    },
