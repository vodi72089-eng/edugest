...
export async function POST(req: NextRequest) {
  try {
    ...
    if (metaToken && metaToken.trim()) {
      // SÉCURITÉ : le token Meta est une clé d'API sensible — il est chiffré
      // AES-256-GCM au repos (comme les clés des passerelles de paiement).
      const encrypted = encryptSecret(metaToken.trim());
      updateData.whatsappMetaToken = encrypted || metaToken.trim();
    }
