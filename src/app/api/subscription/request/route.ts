...
// La demande d'upgrade est une décision de direction : réservée aux admins
// (l'UI « Mon Abonnement » est elle-même réservée à l'admin créateur).
const SUBSCRIPTION_REQUEST_ROLES = ['SUPER_ADMIN_GLOBAL', 'SCHOOL_ADMIN'];
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!SUBSCRIPTION_REQUEST_ROLES.includes(user.role)) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!SUBSCRIPTION_REQUEST_ROLES.includes(user.role)) {
      return NextResponse.json({ error: 'Seuls les administrateurs peuvent demander un changement de forfait' }, { status: 403 });
    }
